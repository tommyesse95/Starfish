...
d_plate = 2e-3 # tba
pd = np.array([2.0, 2.5, 3.0, 3.5])# Torr cm  TBC
# Voltage = np.linspace(250,550,10)
Voltage = np.arange(200,1000,10) # tba  
...
# Check this before simulation 
    ...
    for h in range(len(Voltage)):
        ...
        print(f"Voltage: {Voltage[h]} with dt = {delta_t} s",file=open("output3.txt", "a"))
        ...
        if breakdown:
            print(f"Breakdown! at {time_break} s with {V_break[j]} V", file=open("output3.txt", "a"))
            print(f"Pressure neutral gas: {pressure} Pa", file=open("output3.txt", "a"))
            print(f"Density neutral gas: {n0} per m\u00b3", file=open("output3.txt", "a"))
...
with open('data_sparc3.csv', 'w', newline='') as f: # TBC CHange number file 
