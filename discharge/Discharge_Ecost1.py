...
if os.path.exists('results1'):
    shutil.rmtree('results1') # Initialize the folder of results1
...
d_plate = 2e-3 # tba
pd = np.array([0.5, 0.7, 0.9, 1.3, 1.5, 1.7, 2.0, 2.5, 3.0, 3.5])# Torr cm  TBC
...
for j in range(len(pd)):
    print(f"\n\nSimulation {j+1} with pd = {pd[j]} Torr cm \n",file=open(os.path.join("results1", "output.txt"), "a"))
...
# Check this before simulation
    ...
    for h in range(len(Voltage)):
        ...
        print(f"Voltage: {Voltage[h]} with dt = {delta_t} s",file=open(os.path.join("results1","output.txt"), "a"))
        ...
        print(f"Percentual exceed: {perc_exceed[h]}  %", file=open(os.path.join("results1","output.txt"), "a"))
        with open(os.path.join('results1','perc_exceed.csv'), 'a', newline='') as f:
            thewriter = csv.writer(f)
            thewriter.writerow([pd[j],Voltage[h],perc_exceed[h]])

    if breakdown:
        print(f"Breakdown! at {time_break} s with {V_break[j]} V", file=open(os.path.join("results1","output.txt"), "a"))
        print(f"Pressure neutral gas: {pressure} Pa", file=open(os.path.join("results1","output.txt"), "a"))
        print(f"Density neutral gas: {n0} per m\u00b3", file=open(os.path.join("results1","output.txt"), "a"))
...
with open(os.path.join('results1','data_sparc.csv'), 'a', newline='') as f:
    thewriter = csv.writer(f)
